...
void Rules() {
	...
	for (int i = 0; i < ShipsHeight; i++) {
		...
		for (int j = 0; j < ShipsWidth; j++) {
			if (ShipsInfo[i][j] == '#') { SetColor(COLOR::black, COLOR::blue);	cout << u8"█"; }
...
void PrintTitle(string* Title, int height, int width, int row, int col) {
	for (int i = 0; i < height; i++) {
		...
		for (int j = 0; j < width; j++) {
			if (Title[i][j] == '{') { SetColor(COLOR::black, COLOR::blue);	cout << u8"╔"; }
			else if (Title[i][j] == '}') { SetColor(COLOR::black, COLOR::blue);	cout << u8"╗"; }
			else if (Title[i][j] == '[') { SetColor(COLOR::black, COLOR::blue);	cout << u8"╚"; }
			else if (Title[i][j] == ']') { SetColor(COLOR::black, COLOR::blue);	cout << u8"╝"; }
			else if (Title[i][j] == '-') { SetColor(COLOR::black, COLOR::blue);	cout << u8"═"; }
			else if (Title[i][j] == '|') { SetColor(COLOR::black, COLOR::blue);	cout << u8"║"; }
